...
function ShoppingCartPage(props) {
  ...
  for (const melon in savedCart) {
    const melonObj = props.melons[melon];
    const melonName = melonObj.name;
    const melonQty = props.cart[melon];
    const melonTotal = melonQty * melonObj.price;
    totalPrice += melonTotal;
    cartMelons.push(
      <tr key={melon}>
        <td>{melonName}</td>
        <td>{melonQty}</td>
        <td>{melonTotal.toFixed(2)}</td>
      </tr>
    )
  }
  return (
    <React.Fragment>
      ...
      <div className="col-6">
        <table className="table">
          ...
          <tbody>{cartMelons}</tbody>
        </table>
        <p className="lead">Total: ${(totalPrice).toFixed(2)}</p>
      </div>
    </React.Fragment>
  );
}
...
function Loading() {
  return (
    <div className="loading-div">
      <img src="static/img/watermelon-loading.png">
      </img>
    </div>
  );
}
